...
__kernel void shade(__global Ray *rays, __constant Triangle *triangles,
  ...
  if (I > 0) {
    ...
    if (m.isEmissive) {
      // printf("Hit light.\n");
      intermediate[rays[threadIdx].pixel] *=
      ...
      if (k > 0.00001f) {
        ...
        newRays[old] = newray;
        if (hit_back) {
          intermediate[rays[threadIdx].pixel].x *=
              exp(-m.absorption.x * traveled);
          intermediate[rays[threadIdx].pixel].y *=
              exp(-m.absorption.y * traveled);
          intermediate[rays[threadIdx].pixel].z *=
              exp(-m.absorption.z * traveled);
        }
      }
    } else {
      if (r > d) {
